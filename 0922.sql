...
select 고객아이디,제품명,단가 from 고객 c,주문 o,제품 p where c.고객이름='고명석' and c.고객아이디 = o.주문고객 and p.제품번호=o.주문제품;
...
select 제품명, 단가 from 제품 
                        where 재조업체 = (select 재조업체 from 제품 where 제품명 = '달콤비스킷');
                        
--주문테이블에서 쿵떡파이를 주문한 주문고객, 주문제품, 수량을 검색하시오.
select 주문고객, 주문제품, sum(수량) from 주문 
                                where 주문제품 = (select 제품번호 from 제품 where 제품명 ='쿵떡파이') group by 주문고객,주문제품;
...
--다중행 결과를 나타내는 SubQuery(비교연산자 사용불가능)
--banana 고객이 주문한 제품의 제품번호, 제품명, 제조업체를 검색하시오.
select 주문제품 from 주문 
            where 주문고객='banana';
            
select 제품번호,제품명 제조업체 from 제품 
                        where 제품번호 in(select 주문제품 from 주문 
                                    where 주문고객='banana');
                                    
--김씨 성을 가진 고객의 고객아이디, 나이, 적립금, 주문한 제품명, 단가를 검색하시오.
select 고객아이디, 나이, 적립금, 제품명, 단가 from 고객,제품,주문 
                                        where 고객이름 like '김%' and 고객아이디=주문고객 and 주문제품 = 제품번호;
                                        
-- SubQuery 다중행결과에 사용하는 Not In 연산자    
--banana 고객이 주문하지 않은 제품의 제품명, 제조업체를 검색하시오.
select 제품명,재조업체 from 제품 where 제품번호 not in (select 주문제품 from 주문 where 주문고객='banana');

--대한식품이 제조한 모든 제품의 단가보다 비싼 제품의 제품명, 단가, 제조업체를 검색하시오.
select  제품명, 단가, 재조업체 from 제품;
select 제품명, 단가, 재조업체 from 제품 where 단가 > ALL(select 단가 from 제품 where 재조업체='대한식품');

--2022년 3월 15일에 제품을 주문한 고객의 고객이름을 검색하시오.(1명인경우)
select * from 주문;
select 주문고객 from 주문 where 주문일자 = '2022-03-15';
select 고객이름 from 고객 
                where 고객아이디=(select 주문고객 from 주문 where 주문일자 = '2022-03-15');
                
--2022년 1월 1일에 제품을 주문한 고객의 고객이름을 검색하시오.(여러명인경우)
select 고객이름 from 고객 
                where exists(select 주문고객 from 주문 where 주문일자 = '2022-01-01' and 주문고객=고객아이디);       
                
--2022년 1월 1일에 제품을 주문한 고객이 아닌 고객이름을 검색하시오.(여러명인경우)
select 고객이름 from 고객 
                where not exists(select 주문고객 from 주문 where 주문일자 = '2022-01-01' and 주문고객=고객아이디);                  